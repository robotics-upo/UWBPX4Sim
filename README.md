...
`offboard_launch.py` reads the master layout YAML and launches:

- one `uav_offboard_control_<id>` node per UAV
- one `agv_offboard_control_<id>` node per UGV
- one `clock_publisher` node

The ROS parameters for these nodes are not stored in a separate generated file anymore. They are derived directly from the `offboard:` blocks in the layout YAML.
