...
UWB_LAYOUT_FILE="${UWB_LAYOUT_FILE:-$SCRIPT_DIR/config/uwb_layout.four_vehicle_example.yaml}"
...
tmux send-keys -t $SESSION:1.0 "$(build_ros_shell_command "sleep 12 && cd \"$ROS_WS\" && PX4_SIM_OFFBOARD_BRIDGE_CONFIG=\"$UWB_BRIDGE_CONFIG\" ros2 launch px4_sim_offboard uwb_bridge_launch.py")" C-m
tmux send-keys -t $SESSION:1.1 "$(build_ros_shell_command "sleep 25 && cd \"$ROS_WS\" && UWB_LAYOUT_FILE=\"$UWB_LAYOUT_FILE\" ros2 launch px4_sim_offboard offboard_launch.py")" C-m
